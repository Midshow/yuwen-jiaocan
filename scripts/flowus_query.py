...
# 若作为 skill scripts 运行（无 flowus_data 兄弟目录），回退到 cache 数据目录
_HERE = os.path.dirname(os.path.abspath(__file__))
for cand in [os.path.join(_HERE, '..', '..', '..', '..', 'cache', 'documents', 'flowus_data', '高中语文教参.db'),
             os.path.join(os.path.dirname(_HERE), 'flowus_data', '高中语文教参.db')]:
    if os.path.exists(cand):
        DB = cand; break
else:
    DB = os.path.join(_HERE, 'flowus_data', '高中语文教参.db')
