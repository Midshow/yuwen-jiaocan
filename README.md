...
### 🖥️ 方式一：网页版（推荐给同学——双击即开，零命令）

下载 **`yuwen-jiaocan.html`**（4.6MB 单文件），用浏览器（Chrome/Edge/手机自带浏览器）双击打开即用：

- 左侧选册 → 展开单元 → 点「课文解说」看解读
- 顶部搜索框输入关键词（如「劝学」「意象」「文学短评」）即全文搜索
- 文言文标准译文在「资料链接」→「文言文参考译文」
- 手机可用，可 QQ 直传

### 💻 方式二：命令行（需 Python，零第三方依赖）
